...
class SearchMemberDataSource extends DataGridSource {
  ...
  // Legacy grid helper kept aligned with the four-calendar-month rule.
  bool _isRecentDonation(String? lastDateStr) {
    if (lastDateStr == null || lastDateStr.isEmpty) return false;
    final lastDate = DonorEligibility.parseDonationDate(lastDateStr);
    if (lastDate == null) return false;
    final nextEligible = DonorEligibility.addCalendarMonths(
      DateTime(lastDate.year, lastDate.month, lastDate.day),
      DonorEligibility.waitingPeriodMonths,
    );
    final now = DateTime.now();
    final today = DateTime(now.year, now.month, now.day);
    return today.isBefore(nextEligible);
  }
  ...
  DataGridRowAdapter? buildRow(DataGridRow row) {
    ...
    return DataGridRowAdapter(
        ...
        cells: row.getCells().map<Widget>((dataGridCell) {
          ...
          final cellBackgroundColor =
              isRecentDonation ? Colors.red.shade50 : null;
            ...
            return Container(
              ...
              child: Text(
                ...
                style: TextStyle(
                  fontSize: 12,
                  color:
                      formattedDate.isNotEmpty ? Colors.black87 : Colors.grey,
                  fontWeight:
                      isRecentDonation ? FontWeight.w500 : FontWeight.normal,
                ),
                overflow: TextOverflow.ellipsis,
              ),
            );

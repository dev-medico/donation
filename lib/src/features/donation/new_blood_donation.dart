...
class NewBloodDonationState extends State<NewBloodDonationScreen> {
  ...
  List<String> allIDs = <String>[];
  List<String> allBTypes = <String>[];
  List<String> allBDates = <String>[];
  List<String> allBBCards = <String>[];
  ...
  Future<void> addDonation(
      ...
      String quarter,
      String township) {
    //parse date time now to string format "dd-MMM-yyyy"
  ...
  void setMember(String name) {
    print(name);
    for (var i = 0; i < allMembers.length; i++) {
      if (allIDs[i] == name.split("(")[1].split(")")[0]) {
        setState(() {
          member_id = allIDs[i];
          member_blood_type = allBTypes[i];
          member_blood_bank = allBBCards[i];
          member_birth_date = allBDates[i];
          member_father_name = allFatherNames[i];
          print(member_id);
        });
      }
    }

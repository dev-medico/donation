...
class RequestGiveDetailScreenNew extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<RequestGiveDetailScreenNew> createState() => _RequestGiveDetailScreenNewState();
}

class _RequestGiveDetailScreenNewState extends ConsumerState<RequestGiveDetailScreenNew> {
  int _yearSelected = 0;
  int? _monthSelected;
  bool showYearlyView = true;
  
  List<String> years = List.generate(13, (index) => (DateTime.now().year - index).toString());
          ...
          IconButton(
            ...
            onPressed: () {
              setState(() {
                ...
                if (!showYearlyView && _monthSelected == null) {
              ...
              itemBuilder: (context, index) {
                return GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: _yearSelected == index ? primaryColor : Colors.white,
                      borderRadius: BorderRadius.circular(8),
                      border: Border.all(
                        color: _yearSelected == index ? primaryColor : Colors.grey.shade300,
                      ),
                    ),
                    child: Center(
                      child: Text(
                        years[index],
                        style: TextStyle(
                          color: _yearSelected == index ? Colors.white : Colors.black87,
                          fontWeight: _yearSelected == index ? FontWeight.bold : FontWeight.normal,
                        ),
                      ),
                    ),
                  ),
                ...
                itemBuilder: (context, index) {
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: _monthSelected == index ? primaryColor : Colors.grey.shade200,
                        borderRadius: BorderRadius.circular(8),
                      ),
                      child: Center(
                        child: Text(
                          months[index],
                          style: TextStyle(
                            color: _monthSelected == index ? Colors.white : Colors.black87,
                            fontWeight: _monthSelected == index ? FontWeight.bold : FontWeight.normal,
                            fontSize: 13,
                          ),
                        ),
                      ),
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              children: [
                                Icon(Icons.call_received, color: Colors.orange, size: 16),
                                SizedBox(width: 4),
                                Text(
                                  'တောင်းခံမှု',
                                  style: TextStyle(
            ...
            child: monthlyData.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final request = monthData['totalrequest'] ?? monthData['totalRequest'] ?? 0;
                      final give = monthData['totalgive'] ?? monthData['totalGive'] ?? 0;
                      ...
                      return ListTile(
                        ...
                        subtitle: Row(
                          children: [
                            ...
                            SizedBox(width: 4),
                            Text(request.toString(), style: TextStyle(color: Colors.orange)),
                            SizedBox(width: 16),
                            Icon(Icons.volunteer_activism, size: 14, color: Colors.green),
                            SizedBox(width: 4),
                            Text(give.toString(), style: TextStyle(color: Colors.green)),
                          ],
                        ),
                        trailing: IconButton(
                          icon: Icon(Icons.edit, color: primaryColor, size: 20),
                          onPressed: () => _showAddEditDialog(monthData, month: month, year: year),
                        ),
                      );
                    },
                  ),
  ...
  Widget _buildMonthlyView(Map<String, dynamic> data, int year, int monthIndex) {
  ...
  void _showAddEditDialog(Map<String, dynamic>? existingData, {int? month, int? year}) async {
    final selectedYear = year ?? int.parse(years[_yearSelected]);
    final selectedMonth = month ?? (_monthSelected != null ? _monthSelected! + 1 : DateTime.now().month);
    
    final requestController = TextEditingController(
      text: existingData != null 
        ? (existingData['totalrequest'] ?? existingData['totalRequest'] ?? existingData['request'] ?? 0).toString()
        : '0'
    );
    final giveController = TextEditingController(
      text: existingData != null 
        ? (existingData['totalgive'] ?? existingData['totalGive'] ?? existingData['give'] ?? 0).toString()
        : '0'
    );
            ...
            onPressed: () async {
              ...
              try {
                ...
                final requestGiveData = {
                  ...
                  'date': '${selectedYear.toString().padLeft(4, '0')}-${selectedMonth.toString().padLeft(2, '0')}-01',
                };
